 ...
 ******************************************************************************/
static int bytes2hexstr(const char * bin_in, size_t input_length, char * hex_out, size_t output_length);
static int bytes2base64(const char * input_data, size_t input_length, char * base64_data, size_t output_length);
static int get_passphrase_from_ta(
    char ** passphrase,
    ...
    string_encoding_t string_encoding)
{
    ...
    rc = uta.derive_key(
        uta_context, (unsigned char *)key, TA_KEY_BYTES, (unsigned char *)dv_padded, UTA_LEN_DV_V1, key_slot);
    if (UTA_SUCCESS != rc) {
        free(uta_context);
        return 1;
    }
