...
static void * thread_test_2(void * arg);

/* Define the test cases */
test_case_t test_cases[] = {test_read_version, test_read_uuid, run_self_test, test_trng, test_derive_key, 0};
...
static int test_trng(uta_context_v1_t * uta_context)
{
    ...
    for (j = 0; j < CHI2_NUM_REPEATS; j++) {
        ...
        for (i = 0; i < 16; i++) {
            chisquared += (histogram[i] - CHI2_N_SAMPLES / 16.0) * (histogram[i] - CHI2_N_SAMPLES / 16.0) /
                          (CHI2_N_SAMPLES / 16.0);
        }
...
static int test_derive_key(uta_context_v1_t * uta_context)
{
    ...
    for (i = 0; i < NR_VEC; i++) {
        ...
        for (j = 0; j < USED_KEY_SLOTS; j++) {
            ...
            rc = uta.derive_key(uta_context, ta_output, 32, deriv_value, UTA_LEN_DV_V1, j);
            if (rc != UTA_SUCCESS) {
            ...
            if (key_slots[j] != NULL) {
                 ...
                 * compared.
                 */
                (void)mbedtls_md_hmac(
                    sha256_hmac, key_slots[j], KEYLEN, (unsigned char *)deriv_value, DVLEN, ref_output);
...
static int test_read_uuid(uta_context_v1_t * uta_context)
{
    ...
    if (ref_uuid_set == 0) {
        printf(
            "Setting reference UUID: %02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x\n",
            (unsigned int)uuid[0],
            (unsigned int)uuid[1],
            (unsigned int)uuid[2],
            (unsigned int)uuid[3],
...
static int test_read_version(uta_context_v1_t * uta_context)
{
    ...
    if (print_version == 1) {
        printf(
            "HARDWARE: %u, VERSION: %u.%u.%u\n",
            (unsigned int)version.uta_type,
